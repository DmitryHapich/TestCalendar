...
final class GlassTabBarView: UIView {
    ...
    init(
        ...
        fabGlassEffect.tintColor = .tcMenuBG
        ...
        if let sysImage = action.systemImage {
            ...
            let buttonImage = UIImage(systemName: sysImage, withConfiguration: config)
            button.setImage(buttonImage, for: .normal)
            button.tintColor = .white
            
        } else if let resource = action.image {
            let buttonImage = UIImage(resource: resource)
            button.setImage(buttonImage, for: .normal)
            button.layer.cornerRadius = Constants.barHeight / 2
    ...
    private func setupViews(action: FabBarAction) {
        ...
        NSLayoutConstraint.activate([
            ...
            fabButton.leadingAnchor.constraint(equalTo: fabGlassView.contentView.leadingAnchor, constant: 2),
            fabButton.trailingAnchor.constraint(equalTo: fabGlassView.contentView.trailingAnchor, constant: -2),
            fabButton.topAnchor.constraint(equalTo: fabGlassView.contentView.topAnchor, constant: 2),
            fabButton.bottomAnchor.constraint(equalTo: fabGlassView.contentView.bottomAnchor, constant: -2),
        ])
